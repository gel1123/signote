...
### http-equiv属性
プラグマ指示子として、キャッシュするかどうかなどの指定もできる。

```html
<!-- ブラウザに文書をキャッシュさせないようにする -->
...
## CSSセレクタの詳細度
CSSセレクタが競合したとき、「詳細度」と呼ばれる値の高いものから優先して、CSSを適用する。

詳細度の値は3桁の数字から算出され、桁ごとに値を算出するルールが異なる。

### 詳細度の計算方法
|桁数|計算方法|
|----|--------|
|1桁目|「タイプセレクタ」「疑似要素」の数<br>※疑似要素とは「::before」「::after」「::first-letter」「::first-line」などCSS上で擬似的な要素として扱っている特定の部分のこと|
|2桁目|「クラスセレクタ」「擬似クラスセレクタ」「属性セレクタ」の数<br>※疑似クラスとは「:link」「:visited」「:hover」など擬似的なクラスとしてCSS上で扱っている特定の状態のこと|
|3桁目|idセレクタの数|
...
### base要素
HTML中にひとつだけ、head内に記述出来る要素。相対パスの基準となるURLを定義できる。target属性も持っているので、デフォルトのtargetを指定するのにも使えそう
...
* 書字方向の異なる文字列を混ぜてHTML上で表示するとき、表示がおかしくならないよう、異なる書字方向のエリアを分離させ、周囲のテキストに影響を及ぼさないようにするための要素として、----「bdi要素」----がある
* CSS3のanimationプロパティを使うには、まず「@keyframe」を指定してやる必要がある。@keyframeはアニメーションの開始時点（'0%' or 'from'）の状態と、終了時点（'100%' or 'to'）の状態の定義が必須。
...
## 合格したので、かかった期間や勉強量についてメモ

勉強開始から合格まで 2020-12-01 から 2020-12-09.

このうちしっかり勉強に費やした時間はざっくり1週間。試験当日までにping-tをひとまず1週して、当日には2周目を穴ないか確認しながら問いていった。

合格ライン70点に対し、91点で合格。満点が何点だったのかについては未確認（100点とのことらしいではある）。 合格したので、かかった期間や勉強量についてメモ

昨年、なんとなくで購入した受験チケットが有効期限間近（どころか試験予約日は有効期限の最後の日）だったので受験したが、どうやら調べてみたところ、Level2の試験はJavaScript中心の試験らしい。

JavaScriptはわりと好き（できるとは言っていない）なので、Level2も受けてみることにする。
